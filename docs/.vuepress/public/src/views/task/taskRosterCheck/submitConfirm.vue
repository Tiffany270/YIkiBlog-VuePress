...
import { batchUploadProcess, getCheckPage, exportFailedExcel, uploadCount, deleteByTaskId } from '@/apis/taskManage'
    ...
    submit() {
      if (this.failCount) {
        this.$confirm(`点击确定后提交失败的数据会被清除，若有需要可先导出失败数据再点击完成！`, '提示', {
          ...
          deleteByTaskId({ taskId: this.taskId })
            .then(() => {
              msg.close()
        ...
        deleteByTaskId({ taskId: this.taskId }).then(() => {
          this.$message.success('提交成功')
          this.goBack()
        })
      }
