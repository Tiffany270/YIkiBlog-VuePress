        ...
        <div v-if="displayUrlList.length < limitCount">
          ...
          >
            <div class="upload-bg">
              ...
              <div class="upload-tips" :style="{ marginTop: limitCount > 1 ? '-20px' : '-10px' }">
                拖拽或<span>点击上传</span>
              </div>
              <div class="upload-tips" :style="{ marginTop: limitCount > 1 ? '-20px' : '-10px' }" v-if="limitCount > 1">
                最多上传<span>{{ limitCount }}</span
                >张图片
              </div>
            </div>
          </el-upload>
